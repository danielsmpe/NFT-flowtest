...
transaction() {
  ...
  prepare(signer: AuthAccount) {
    ...
    */
    destroy signer.storage.load<@{NonFungibleToken.Collection}>(from: ExampleNFT.CollectionStoragePath)
    ...
    if signer.storage.borrow<&ExampleNFT.Collection>(from: ExampleNFT.CollectionStoragePath) == nil {
      ...
      <&ExampleNFT.Collection{NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection}>(ExampleNFT.CollectionPublicPath, target: ExampleNFT.CollectionStoragePath)
    }
  }
